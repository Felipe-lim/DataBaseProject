...
if page == "Start":
    ...
    # Adicionando os widgets à barra lateral
    user_type = st.sidebar.radio(
        "Escolha o tipo de usuário", ["Cliente", "Fornecedor", "Funcionário"]
    )

    # Dropdown na barra lateral para selecionar a operação CRUD
    operation = st.sidebar.selectbox(
        "Escolha sua operação", ["Create", "Read", "Update", "Delete"]
    )

    # Conteúdo principal baseado na operação CRUD selecionada
    if operation == "Create":
        ...
        if read_option == "Buscar usuário específico":
            ...
            if st.button("Obter informações"):
                if validate_input({"Identificador": identifier}):
                    db = next(get_db())
                    try:
                        # Buscar usuário e pessoa associados ao CPF/CNPJ
                        user, pessoa = find_user_and_pessoa(db, identifier, user_type)

                        if user and pessoa:
                            # Preparar os dados para exibição como strings
                            data = {
                                "ID": str(pessoa.id),  # Convertendo UUID para string
                                "Nome": pessoa.nome,
                            ...
                            # Convertendo todos os atributos do usuário para strings
                            for k, v in user.__dict__.items():
                                if not k.startswith("_"):
                                    data[k] = str(v)  # Converte cada valor para string

                            # Exibir os dados como dicionário para evitar problemas com Arrow
            ...
            try:
                ...
                if pessoas:
                    data = [
                        {
                            "ID": str(p.id),  # Convertendo UUID para string
                            "Nome": p.nome,
                            "Endereço": p.endereco,
                            "Email": p.email,
                            "Telefone": p.telefone,
                    ...
                    # Garantindo que todas as colunas do DataFrame são strings
                    df = df.astype(str)
